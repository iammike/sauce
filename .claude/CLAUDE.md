...
- The fructose ratio lives in an inline `.advanced` disclosure. It's the one input that meaningfully alters a tested formula, so it shouldn't sit inline inviting a nudge — but it stays fully wired, not read-only
