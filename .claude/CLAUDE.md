...
- A test that reproduces a figure the page shows must call the page's own function, not recompute it. `mixCostPerGramCarb()` exists in `src/cost.js` for that reason — it was inline in `renderCost()`, and a test that recomputed the same number dropped the per-bottle flavouring term, invisible while no default is `perBottle` and wrong the moment one is
...
- The ratio control reads **glucose-first** (`Glucose : fructose`, a fixed `1 :` then the field), because that is the glucose-first convention a packet uses — `1:0.8`, `1:1`. Not universal: a 0.5 mix is sold as `2:1`, which is what `RATIO_MARKET_NAMES` is left for. Reading the way people already know it is what let the explanatory ADVANCED note go, along with `.field-pair` and `.field--note`, which existed only to carry it. The stored number is the same either way round: 0.8 is the fructose side of both `0.8:1` and `1:0.8`, so flipping the presentation needed no persistence or share-token migration
- The `1 :` is decoration and `aria-hidden`, so the input carries an `aria-label` supplying the missing term. It repeats the visible label verbatim first, or WCAG 2.5.3 (Label in Name) fails for voice control
- `RATIO_MARKET_NAMES` now names a ratio only where the market disagrees with what the control already shows. 0.5 is the one that does — universally sold as `2:1`, never `1:0.5`
...
- **There is no standalone osmolality note on the page any more.** One was added in #27 and removed as more than a reader needs from a cost panel. The mechanism still has to appear where it does work — regular Gatorade's `limitation` carries the figures and there's a test on it — and the full derivation lives in `docs/recipe-source.md`. Don't reintroduce a general explainer; put the fact next to the thing it explains

## The carb ratio: stored fructose-first, shown glucose-first (#26)

- `carbRatio` is **grams of fructose per 1 g of maltodextrin** — the fructose-first form the literature uses. That's the *stored* number and it must stay that way: flipping it to an `X:1` glucose-first value would need a persistence and share-token migration, and `1:0` (glucose only) has no `X:1` equivalent. How it's *presented* is a separate question, answered under "Horizontal space" — the control shows the same number glucose-first, because 0.8 is the fructose side of both `0.8:1` and `1:0.8`
