...
- Resolve a `#calc-form` select exactly once, in `readInputs()`, and thread the resolved value through — don't re-read `$(id).value` a second time elsewhere in the render chain. `renderSaltNote()` used to re-resolve `in-salt-profile` independently and could disagree with `readInputs()`'s fallback: the batch would render at the endurance numbers while the note quietly went blank, with no error and no indication the two had diverged
